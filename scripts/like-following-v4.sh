...
# 函数：检查标题是否包含 emoji
# 使用 Unicode 范围检测常见 emoji
has_emoji() {
  local text="$1"
  # 检测常见 emoji 范围：U+1F300-U+1F9FF (大部分 emoji)
  # 也检测 U+2600-U+26FF, U+2700-U+27BF 等
...
for USER_UUID in $(jq -r '.[].uuid' "${ALL_USERS_FILE}"); do
  ...
  npx -y @talesofai/neta-skills@latest request_interactive_feed \
    ...
    2>&1 | sed 's/^\[telemetry\].*//g' | sed 's/^>.*//g' | grep '{' > "${TMP_DIR}/user_posts_raw.json"
  
  # 尝试解析 JSON
  if ! jq '.' "${TMP_DIR}/user_posts_raw.json" > "${TMP_DIR}/user_posts.json" 2>/dev/null; then
    echo "  JSON 解析失败，跳过"
    continue
  fi
  
  # 提取帖子列表
  MODULE_COUNT=$(jq '.module_list | length' "${TMP_DIR}/user_posts.json")
  
  if [ "${MODULE_COUNT}" -eq 0 ] || [ "${MODULE_COUNT}" = "null" ]; then
    echo "  无帖子"
    continue
  fi
  ...
  for i in $(seq 0 $((MODULE_COUNT - 1))); do
    STORY_UUID=$(jq -r ".module_list[${i}].json_data.uuid" "${TMP_DIR}/user_posts.json")
    STORY_TITLE=$(jq -r ".module_list[${i}].json_data.name" "${TMP_DIR}/user_posts.json")
    LIKE_STATUS=$(jq -r ".module_list[${i}].json_data.likeStatus" "${TMP_DIR}/user_posts.json")
