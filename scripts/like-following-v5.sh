...
for USER_UUID in $(jq -r '.[].uuid' "${ALL_USERS_FILE}"); do
  ...
  # 注意：需要使用 interactive_feed API
  if [ -n "${BIZ_TRACE_ID}" ]; then
    FEED_RESPONSE=$(curl -s -X POST "https://api.talesofai.cn/v3/feed/interactive" \
      -H "x-token: ${TOKEN}" \
      -H "x-platform: nieta-app/web" \
      -H "Content-Type: application/json" \
      -d "{\"page_index\":0,\"page_size\":10,\"scene\":\"personal_feed\",\"target_user_uuid\":\"${USER_UUID}\",\"biz_trace_id\":\"${BIZ_TRACE_ID}\"}")
  else
    FEED_RESPONSE=$(curl -s -X POST "https://api.talesofai.cn/v3/feed/interactive" \
      -H "x-token: ${TOKEN}" \
      -H "x-platform: nieta-app/web" \
      -H "Content-Type: application/json" \
      -d "{\"page_index\":0,\"page_size\":10,\"scene\":\"personal_feed\",\"target_user_uuid\":\"${USER_UUID}\"}")
  fi
  ...
  for i in $(seq 0 $((MODULE_COUNT - 1))); do
    ...
    LIKE_RESULT=$(curl -s -X POST "https://api.talesofai.cn/v3/collection/like" \
      -H "x-token: ${TOKEN}" \
      -H "x-platform: nieta-app/web" \
      -H "Content-Type: application/json" \
