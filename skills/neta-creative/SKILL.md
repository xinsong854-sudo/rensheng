...
```bash
npx -y @talesofai/neta-skills@latest edit_collection --uuid "collection uuid" --name "new name" --description "new description" --tags "tag1,tag2" --status "PUBLIC"|"PRIVATE"
```

Edit an existing collection (name, description, tags, status, etc.). Only provided fields will be updated.

**Publish collection**

```bash
npx -y @talesofai/neta-skills@latest publish_collection --uuid "collection uuid"
```

Publish or update a collection. Makes the collection visible to others.
